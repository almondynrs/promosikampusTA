...
@section('content')
            ...
            <div class="container">
                <div class="row">
                    <div class="col-xl-12 col-lg-12 col-md-12 d-flex flex-column mx-auto">
                        ...
                        <form action="{{ route('general.saveSessionQM') }}" method="POST" id="formq">
                            @csrf
                            @php
                                $num = 1;
                            @endphp
                            @foreach ($perbandingan as $a1 => $p1)
                                        ...
                                        <div class="card m-2">
                                            <div class="card-body">
                                                <div class="table-responsive p-0">
                                                    <table class="table align-items-center mb-0">
                                                        ...
                                                        <tbody class="text-center">
                                                            <tr>
                                                                <td>
                                                                    <label for="p-1"
                                                                        class="form-label col-12 col-lg-2">{{ $p1->nama_kriteria }}</label>
                                                                </td>
                                                                <td>
                                                                ...
                                                                onchange="resetNextSliders(this)" name="inputAtas_{{$a1}}" multiple> --}}
                                                                    <input type="range" value="0"
                                                                        ...
                                                                        name="atas[{{ $p1->id }}][{{ $p2->id }}]"
                                                                        data-num="{{ $num }}"
                                                                        data-lvl="{{ $p1->id }}" multiple>
                                                                    {{-- <input type="hidden" name="id_kriteria1[]" value="{{ $p1->id }}"/> --}}
                                                                </td>
                                                            </tr>
                                                            <tr>
                                                                <td>
                                                                    <label for="p-2"
                                                                        class="form-label col-12 col-lg-2">{{ $p2->nama_kriteria }}</label>
                                                                </td>
                                                                <td>
                                                            ...
                                                            onchange="resetNextSliders(this)" name="inputBawah_{{$a1}}" multiple> --}}
                                                                    <input type="range" value="0"
                                                                        class="form-range w-100" min="0"
                                                                        max="9"
                                                                        id="p-{{ $p2->id }}-{{ $p1->id }}"
                                                                        ...
                                                                        oninput="updateSliderValue(this)"
                                                                        onchange="resetNextSliders(this)"
                                                                        name="bawah[{{ $p1->id }}][{{ $p2->id }}]"
                                                                        data-num="{{ $num }}"
                                                                        data-lvl="{{ $p2->id }}" multiple>
                                                                    {{-- <input type="hidden" name="id_kriteria2[]" value="{{ $p2->id }}"/> --}}
                                                                </td>
                                                            </tr>
                                                    ...
                                                    <input type="hidden"
                                                        name="perbandingan-[{{ $p1->id }}][{{ $p2->id }}]"
                                                        id="perbandingan-[{{ $p1->id }}][{{ $p2->id }}]"
                                                        value="{{ $p1->id }}">
                                                    <input type="hidden"
                                                        name="perbandingan-[{{ $p1->id }}][{{ $p2->id }}]"
                                                        id="perbandingan-[{{ $p1->id }}][{{ $p2->id }}]"
                                ...
                                @endforeach
                            @endforeach
                            <input type="hidden" name="id_list" value="{{ $quisioner->listpertanyaan[0]->id }}">
    ...
    <script type="text/javascript">
        ...
        function showAlert(event) {
            ...
            for (var i = 1; i < {{ count($perbandingan) }} + 1; i++) {
                for (var j = 1; j < {{ count($perbandingan) }} + 1; j++) {
                    if (j > i) {
                        var j1 = document.getElementById('p-' + i + '-' + j);
                        var j2 = document.getElementById('p-' + j + '-' + i);

                        if (j1.value == 0 && j2.value == 0) {
                            elementToFocus = j1;
                            shouldSubmit = false;
                            break;
                        }
